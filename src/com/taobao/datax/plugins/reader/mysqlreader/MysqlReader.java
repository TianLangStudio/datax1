...
public class MysqlReader extends Reader {
	...
	@Override
	public int connect() {
		/*conn = DBSource.getConnection(this.getClass(), ip, port, dbname,username,password);*/
		return PluginStatus.SUCCESS.value();
	}
	...
	public List<PluginParam> split(PluginParam param){
        ...
        Connection conn = null;
		try {
            logger.info("getCon begin");
            conn = getCon();
            logger.info("getCon end");
			//conn = DBSource.getConnection(this.getClass(), ip, port, dbname,username,password);
            if (sql.indexOf(" limit ") < 0) {
                sql = sql + " limit 1";
            }
            logger.info("genMetaData begin");
            logger.info("genMetaData sql:" + sql);
			m = DBUtils.genMetaData(conn, sql);
            logger.info("genMetaData end");
