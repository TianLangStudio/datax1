...
public class SqlServerReader extends Reader {
	...
	public int init(){
		...
		this.jdbcUrl = param.getValue(ParamKey.jdbcUrl, String.format("jdbc:sqlserver://%s;DatabaseName=%s", this.ip, this.database));
		this.driverName = param.getValue(ParamKey.driverName, DRIVER_NAME);
	...
	private Properties genProperties() {
		...
		logger.info("driverClassName:" + driverName);
		return p;
	}
	...
	public int connect(){
		try {
			this.connection = DBSource.getConnection(this.getClass(), this.ip, this.port, this.database,this.username,this.password);
		} catch (Exception e) {
			logger.error(ExceptionTracker.trace(e));
			throw new DataExchangeException(e.getCause());
		}
