...
public class FtpReader extends Reader {
    ...
    private  String[] readFileArr;
    private boolean isNotRead(String fileName){

        if(haveReadFile == null || "".equals(haveReadFile)) {
            return  true;
        }

        try {
            if(readFileArr == null) {
                String content = TextFile.readFile(new File(haveReadFile));
                readFileArr = content.split("\n");
                LOG.info("have read file num:" + readFileArr.length);
            }

        } catch (Exception e) {
             LOG.error("read haveReadFile error",e);
             throw new DataExchangeException(e);
        }

        for(String readFile : readFileArr) {
            if(readFile.equals(fileName) || (readFile.endsWith(File.pathSeparator) && fileName.startsWith(readFile))) {
                return false;
            }
        }
    ...
    private void fileHaveRead(String fileName) {
        if("".equals(haveReadFile)) {
            LOG.info("haveReadFile not set");
            return;
        }
        LOG.info("have read file " + fileName);
        TextFile.write2File(haveReadFile, fileName);
    ...
    public void startRead2(LineSender sender) {
       ...
       /* if(!isNotRead(fileName)) {
        ...
        LOG.debug("end read source files..." + fileName);
    ...
    private void validateParameter() {
        ...
        this.interval = param.getIntValue(ParamKey.INTERVAL, Constant.DEFAULT_INTERVAL);
        this.haveReadFile = param.getValue(ParamKey.HAVE_READ_FILE, "");
        if(!"".equals(haveReadFile)) {
            File file = new File(haveReadFile);
            if(!file.exists()) {
                try {
                    file.createNewFile();
                } catch (Exception e) {
                    LOG.error("create have read file error", e);
                    throw new DataExchangeException(e);
                }
            }
            if(file.isDirectory()) {
                String msg = "have read file must be a file not directory";
                LOG.error(msg);
                throw new DataExchangeException(msg);
            }
        }
        this.readFileNum = param.getIntValue(ParamKey.READ_FILE_NUM, Constant.DEFAULT_READ_FILE_NUM);
